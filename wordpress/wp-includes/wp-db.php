...
<p>これは <code>wp-config.php</code> ファイルのユーザー名とパスワードが間違っているか、<code>$dbhost</code> のデータベースサーバーに接続できないかのどちらかを意味します。ホストのデータベースサーバーがダウンしているのかもしれません。</p>
<ul>
	<li>ユーザー名とパスワードに間違いはありませんか ?</li>
	<li>ホスト名に間違いはありませんか ?</li>
	<li>データベースサーバーは動いていますか ?</li>
</ul>
<p>上記の用語がよく分からなければサーバーの管理者に連絡したほうがいいかもしれません。助言が必要であれば <a href='http://wordpress.org/support/'>WordPress サポートフォーラム（英語）</a>を訪れるといいでしょう。</p>
");
		}
...
<h1>データベースを選択できません</h1><p>データベースサーバーへ接続することはできましたが (ユーザー名とパスワードは正しいということです)、<code>$db</code> データベースを選択することができませんでした。</p><ul><li>そのデータベースは作成済みですか ?</li><li>システムによっては、例えば username_wordpress のように、データベース名の前にユーザー名が付加される場合があります。その可能性はありませんか ?</li></ul><p>データベースのセットアップ方法が分からなければ<strong>お使いのサーバー管理者</strong>に聞いてください。もし何をしてもうまくいかなければ、<a href='http://wordpress.org/support/'>WordPress サポートフォーラム（英語）</a>で助言がもらえるかもしれません。</p>");
	...
	function print_error($str = '') {
		...
		if ( $this->show_errors ) {
			...
			print "<div id='error'>
			<p class='wpdberror'><strong>WordPress データベースエラー:</strong> [$str]<br />
			<code>$query</code></p>
			</div>";
		} else {
			return false;
