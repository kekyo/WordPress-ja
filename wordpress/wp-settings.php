...
if ( version_compare( '4.2', phpversion(), '>' ) ) {
	die( 'サーバーの PHP のバージョンは ' . phpversion() . ' です。WordPress は 4.2 以上でご利用になれます。' );
}

if ( !extension_loaded('mysql') && !file_exists(ABSPATH . 'wp-content/db.php') )
	die( 'お使いのサーバーは PHP から MySQL を利用できないようです。' );
...
if ( preg_match('|[^a-z0-9_]|i', $wpdb->prefix) && !file_exists(ABSPATH . 'wp-content/db.php') )
	wp_die("<strong>エラー</strong>: <code>wp-config.php</code> 内の <code>$table_prefix</code> に用いることができるのは半角英数およびアンダースコアのみです。");
